...
第一，建立 Rust 异常控制流相关内存安全漏洞的分类体系。我们将全面梳理 Rust 生态中已知的由于 panic/unwind 引发的漏洞案例和模式，借鉴异常安全理论和 Rust 内存模型，提出一套清晰的分类标准。该体系将精确定义各类恐慌安全漏洞的成因、条件和表现，包括但不限于：因 panic 导致的资源释放错误（如双重释放、释放顺序紊乱）、因 panic 导致的不变量破坏（未初始化数据被误用等）、panic 跨线程或 FFI 边界引发的未定义行为等等。分类体系围绕触发点、资源状态、边界位置等维度构建场景化框架，将问题归纳为四类典型场景（S1–S4），并明确各场景的触发机制、内在原理及典型后果。分类体系的建立，有助于澄清过去语义不明确的问题，为后续分析提供共同语言。总之，此分类体系既覆盖已有文献提出的类别如 Panic Safety，又进一步细化拓展，为 Rust 异常相关漏洞提供系统且有层次的描述框架。
...
（1）面向异常控制流的漏洞分类与建模。本研究首次针对 Rust 语言，将以往零散的“panic 安全”问题提升到系统分类的高度，提出了异常/恐慌相关内存漏洞的场景化分类体系。相比现有文献中对 “Panic Safety” 一语带过，我们的分类更为精细全面，围绕触发点、资源状态、边界位置三个维度构建框架，将问题归纳为四类典型场景（S1–S4），并逐一分析各场景的触发机制、内在原理及典型后果。这一创新在理论上丰富了内存安全漏洞的类别维度，拓展了 Rust 安全模型，使其囊括非常规控制流带来的影响。通过机制级分析与漏洞类型映射，我们能够对 panic 相关漏洞的发生机理进行清晰推演和分类验证。这一创新点为后续分析和工具实现奠定了扎实基础，在学术上具有新颖性和独创性。
...
本文余下章节结构如下。第二章介绍相关背景与技术基础，包括 Rust 内存安全模型与资源管理（所有权、借用、生命周期、RAII 与 Drop、unsafe 语义）、Rust panic 机制与非正常控制流、panic 安全性语义体系，以及 Rust 静态分析技术基础（MIR 等）。第三章综述相关工作并确定评测对象，系统介绍 Rudra、MirChecker、FFIChecker 三款代表性 Rust 静态分析工具的分析原理、适用范围及在 panic 相关问题上的支持情况，并对比其差异与共性，为后续评测设计提供依据。第四章提出 Rust 非正常控制流内存安全问题的场景分类与机制分析，从触发点、资源状态、边界位置三个维度构建分类框架，将问题归纳为四类典型场景（S1：Drop 执行中途 panic；S2：未初始化/部分初始化时 panic；S3：panic 跨 FFI 边界；S4：自定义资源清理逻辑中 panic），并逐一分析各场景的触发机制、内在原理及典型后果。第五章在此基础上构建基于场景分类的静态分析工具评测框架，界定评测目标与研究问题，阐述基准数据集构建策略、评测流程与结果归一化方法，设计涵盖基础指标、场景感知指标与成本指标的指标体系，并提出能力边界分析方法。第六章开展实验与结果分析，在 100 个非正常控制流相关用例上对三款工具进行系统评测，给出总体对比结果、分场景表现及误差分析，并总结跨场景能力边界与改进建议。第七章总结全文工作，归纳主要贡献与结论，分析研究的局限性，并对未来工作方向进行展望。通过上述章节的展开，本文全面呈现 Rust 非正常控制流下内存安全研究从背景铺垫、工具梳理、场景分类、评测框架到实证评估的完整研究链条。
