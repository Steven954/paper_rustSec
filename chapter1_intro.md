...
（1）针对异常控制流相关讨论难以刻画 panic 触发的状态演化、分类表述难与工具评测对齐等局限，本文提出根因驱动的场景分类与解释方法：以状态碎化理论与三阶段演进模型突出 panic 的关键节点角色，按触发位置、资源状态与隔离边界划分 S1–S4，贯通各场景机理、后果及与传统漏洞标签的映射，为风险分析与后续评测提供统一语义锚点。
...
第 3 章 基于状态碎化理论的异常控制流多维场景分类体系（创新点一）。 从程序语言理论与异常安全基本保证出发，形式化刻画状态碎化（逻辑元数据与物理内存脱节）及三阶段威胁演进；结合 MIR 中清理终止符与倒置解卷树说明栈展开路径；在触发位置边界、资源状态与隔离跨越度三维空间内界定 S1–S4，并以决策树论证分类互斥与完备；将各场景与 CWE 建立映射，为后续基准与工具语义对齐提供依据。基于自建用例集的分布与归类实证放在第 4 章展开。
