param(
  ...
  [string]$Target = 'tests/safe-bugs/division-by-zero',
...
if (-not $targetPath) {
  ...
  Write-Host 'Tip: pass a Cargo project directory, or a single .rs file.'
  exit 1
}
...
if ($targetIsDir) {
  $runCmd = "/workspace/target/debug/cargo-mir-checker mir-checker -- $argsStr"
} else {
