...
if (-not $targetPath) {
  ...
  Write-Host 'Tip: pass a .rs file, or a Cargo project directory.'
  exit 1
}
...
if ($targetIsDir) {
  ...
  if (-not (Test-Path $cargoToml)) {
    Write-Error "Target directory does not contain Cargo.toml: $targetPath"
    exit 1
  }
}
...
$targetDir = if ($targetIsDir) { $targetPath } else { Split-Path -Parent $targetPath }
$targetName = if ($targetIsDir) { '' } else { Split-Path -Leaf $targetPath }

$containerRepo = '/workspace/safedrop'
$containerHome = '/workspace/safedrop-home'
$containerTargetRoot = '/workspace/target'
$containerTarget = if ($targetIsDir) { $containerTargetRoot } else { "$containerTargetRoot/$targetName" }

$targetFlag = if ($targetIsDir) { '1' } else { '0' }
...
$bashCmd = $bashCmdTemplate -f $RustTag, $targetFlag, $containerTarget
$innerScript = Join-Path $root 'run_safedrop_inner.sh'
$innerContent = $bashCmd -replace "`r`n", "`n"
[System.IO.File]::WriteAllText($innerScript, $innerContent, (New-Object System.Text.UTF8Encoding($false)))

docker run -t --rm `
  --entrypoint /bin/bash `
  -v "${root}:${containerRepo}" `
  -v "${safedropHome}:${containerHome}" `
  -v "${targetDir}:${containerTargetRoot}" `
  $Image "$containerRepo/run_safedrop_inner.sh"
