...
**Rudra**：检出 34 个（Recall 51.52%）。其中相当一部分为**误报**：包括 `cargo metadata` 依赖解析失败（如 `failed to get macros as a dependency`）、`unused variable`、`unused import`、`struct is never constructed` 等编译器警告。这些输出被解析器误判为“检出”，导致 Recall 虚高。真正与 panic safety 相关的检出如 `UnsafeDataflow`、`SendSyncVariance` 等仅占少数。
